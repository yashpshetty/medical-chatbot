...
VISION_MODEL  = "Qwen/Qwen2.5-VL-7B-Instruct"  # vision model for real image analysis
...
if not st.session_state.logged_in:
    ...
    # ── Render sidebar + messages (no chat_input inside) ─────
    page_chat()
    ...
    # (must be outside page_chat so it fires before chat_input)
    if st.session_state.pending_input:
        pi = st.session_state.pending_input
        st.session_state.pending_input = None
        # Will be processed by send_message below via pending flag reuse
        st.session_state["_send_text"] = pi

    # ── SEND MESSAGE (top-level function, not nested) ─────────
    def send_message(user_text: str,
        ...
        if not user_text.strip() and not doc_text and not image:
            return
        ...
        this_s = next((s for s in sessions_list if s["id"] == sid), None)
    ...
    # Fire pending example-button text
    if st.session_state.get("_send_text"):
        txt = st.session_state.pop("_send_text")
        send_message(txt)
        st.rerun()

    # ── STAGED FILE PREVIEWS ──────────────────────────────────
    staged_image    = st.session_state.get("staged_image")
    staged_doc_text = st.session_state.get("staged_doc_text")
    staged_doc_name = st.session_state.get("staged_doc_name")
    ...
    # Only process when a NEW file is uploaded (not already staged)
    already_staged_name = (
        st.session_state.get("staged_image_name") or
        st.session_state.get("staged_doc_name") or ""
    )
    if uploaded_file and uploaded_file.name != already_staged_name:
        ...
        if ext in ("jpg", "jpeg", "png", "webp", "bmp"):
            ...
            st.session_state.pop("staged_doc_text", None)
            st.session_state.pop("staged_doc_name", None)
            # Refresh previews without infinite loop
            staged_image    = pil
            staged_doc_text = None
            staged_doc_name = None
        else:
            with st.spinner(f"Reading {uploaded_file.name}…"):
                doc_text_extracted, _ = extract_document_text(uploaded_file)
            if doc_text_extracted.strip() and not (
                    doc_text_extracted.startswith("[") and "error" in doc_text_extracted.lower()):
                st.session_state["staged_doc_text"] = doc_text_extracted
                st.session_state["staged_doc_name"] = uploaded_file.name
                st.session_state.pop("staged_image",      None)
                st.session_state.pop("staged_image_name", None)
                staged_image    = None
                staged_doc_text = doc_text_extracted
                staged_doc_name = uploaded_file.name
            else:
                st.error(f"Could not read file: {doc_text_extracted}")

    # ── CHAT INPUT — top-level, always runs ───────────────────
    if staged_doc_text:
