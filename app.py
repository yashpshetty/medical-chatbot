...
def page_chat():
    ...
    # ── INLINE IMAGE ATTACH BAR (above chat input) ───────────────
    ...
    # Inline image takes priority over expander image
    active_image      = inline_image or current_image
    active_image_name = inline_image_name or st.session_state.get("current_image_name", "")

    # ── Compact attach row ────────────────────────────────────────
    # Shows: [📷 Attach image] button  +  thumbnail + ✕ if an image is staged
    st.markdown("""
    <style>
    /* Shrink the file-uploader to a tidy icon button */
    ...
    attach_col, preview_col = st.columns([2, 5])
    ...
    with preview_col:
        if inline_image:
            ...
            with p1:
                st.image(inline_image, caption=f"📷 {inline_image_name}", width=120)
            with p2:
    ...
    # ── CHAT INPUT (always full-width, never blocked) ─────────────
    if current_doc_text:
    ...
    if user_input is not None:
        # Re-read active state at submit time
