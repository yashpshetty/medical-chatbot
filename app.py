...
def medical_bot(user_input: str, chat_history: list,
    ...
    if uploaded_image is not None:
        ...
        img_name = st.session_state.get("current_image_name") or st.session_state.get("inline_image_name", "")
...
def defaults():
    keys = {
        ...
        "pending_input":      None,
        # ── file caches ──────────────────────────────────────────
        "doc_analysis_cache":   None,        # {doc_name, summary} — first analysis result
        "image_analysis_cache": None,        # {image_name, summary} — first image analysis
        "inline_image":         None,        # PIL Image uploaded from chat bar
        "inline_image_name":    "",
    }
...
def page_chat():
    ...
    with st.sidebar:
        ...
        if st.button("➕ New Chat", use_container_width=True, type="primary"):
            ...
            st.session_state.pop("inline_image",       None)
            st.session_state.pop("inline_image_name",  None)
            st.rerun()
    ...
    with st.expander("📎 Upload Medical Image or Document (optional)", expanded=False):
        upload_tab, doc_tab = st.tabs(["🖼️ Medical Image", "📄 Medical Document"])

        # ── IMAGE TAB ──
        with upload_tab:
            st.caption(
                "Upload a photo of a skin condition, rash, wound, or other visible symptom. "
                "Accepted: JPG, PNG, WEBP, BMP."
            )
            ...
            if img_file:
                ...
                st.session_state["current_image_name"] = img_file.name
                # Clear any document when image is loaded
                st.session_state.pop("current_doc_text", None)
                st.session_state.pop("current_doc_name", None)
            else:
        ...
        with doc_tab:
            st.caption(
                "Upload a medical document such as a lab report, prescription, discharge summary, "
                "blood test results, or radiology report. Accepted: PDF, DOCX, TXT."
            )
            doc_file = st.file_uploader(
                "Choose a document",
                type=["pdf", "docx", "txt", "csv", "rtf"],
                key="doc_upload",
                label_visibility="collapsed"
            )
            if doc_file:
                with st.spinner(f"Reading {doc_file.name}..."):
                    doc_text, doc_type = extract_document_text(doc_file)

                if doc_text.startswith("[") and "error" in doc_text.lower():
                    st.error(doc_text)
                    ...
                    st.session_state["current_doc_name"] = doc_file.name
                    # Clear any image when doc is loaded
                    st.session_state.pop("current_image",      None)
                    st.session_state.pop("current_image_name", None)
                    ...
                    if len(doc_text) > 500:
                        preview += "…"
                    st.success(f"✅ **{doc_file.name}** ({doc_type}) — {len(doc_text):,} characters extracted")
                    with st.expander("📋 Document preview", expanded=False):
                        st.text(preview)
                    st.info(
                        "Document loaded! Type your question below — e.g. "
                        "*\"What do these results mean?\"* or "
                        "*\"Are any values abnormal?\"* — or just press Enter "
                        "to get an automatic summary."
                    )
            else:
                st.session_state.pop("current_doc_text", None)
                st.session_state.pop("current_doc_name", None)
        ...
        with st.chat_message(msg["role"], avatar=avatar):
            ...
            if msg.get("image_data"):
                try:
                    ...
                    pass
            # Show document badge if present
            if msg.get("doc_name"):
                st.markdown(
    ...
    def send_message(user_text: str,
        ...
        # Auto-fill question if user hit Enter with a doc loaded but no question
        ...
        # Add user message to display (include doc_name for badge)
        ...
        # Save user message to DB (store doc_name in image_data field as JSON if needed)
        extra = None
        if doc_name:
            extra = json.dumps({"doc_name": doc_name})
        ...
        # Clear expander-uploaded file references after sending
        # (doc_analysis_cache and image_analysis_cache are intentionally kept for follow-ups)
        st.session_state.pop("current_image",      None)
        st.session_state.pop("current_image_name", None)
        st.session_state.pop("current_doc_text",   None)
        st.session_state.pop("current_doc_name",   None)
    ...
    current_doc_text = st.session_state.get("current_doc_text", None)
    current_doc_name = st.session_state.get("current_doc_name", None)
    inline_image     = st.session_state.get("inline_image",     None)
    inline_image_name = st.session_state.get("inline_image_name", "")

    # Merge expander image + inline image (inline takes priority if present)
    active_image      = inline_image or current_image
    active_image_name = inline_image_name or st.session_state.get("current_image_name", "")

    # Show preview of inline image if attached
    if inline_image:
        prev_col, clear_col = st.columns([6, 1])
        with prev_col:
            st.image(inline_image, caption=f"📷 {inline_image_name}", width=160)
        with clear_col:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("✕", key="clear_inline_img", help="Remove image"):
                st.session_state.pop("inline_image",      None)
                st.session_state.pop("inline_image_name", None)
                st.rerun()

    if current_doc_text:
        placeholder = "Ask about the document — e.g. 'Are any values abnormal?' — or press Enter for a summary..."
    elif active_image:
        placeholder = "Ask a question about the image..."
    else:
        placeholder = "Describe your symptoms or ask a health question..."

    # Two-column layout: 📷 uploader | chat input
    img_col, input_col = st.columns([1, 11])
    ...
    with img_col:
        ...
        inline_file = st.file_uploader(
            "📷",
            type=["jpg", "jpeg", "png", "webp", "bmp"],
            key="inline_img_upload",
            label_visibility="collapsed",
            help="Attach an image to your message"
        )
        st.markdown("</div>", unsafe_allow_html=True)
        if inline_file:
            pil_img = Image.open(inline_file)
            st.session_state["inline_image"]      = pil_img
            st.session_state["inline_image_name"] = inline_file.name
            # Clear expander image so they don't conflict
            st.session_state.pop("current_image",      None)
            st.session_state.pop("current_image_name", None)
            st.rerun()
    ...
    if user_input is not None:        # catches empty-string Enter press too
        # Re-read active after possible inline upload
        active_image      = st.session_state.get("inline_image") or st.session_state.get("current_image")
        active_image_name = st.session_state.get("inline_image_name") or st.session_state.get("current_image_name", "")
        if user_input.strip() or current_doc_text or active_image:
            send_message(
                user_input,
                image=active_image,
            ...
            # Clear inline image after sending
            st.session_state.pop("inline_image",      None)
            st.session_state.pop("inline_image_name", None)
            st.rerun()
