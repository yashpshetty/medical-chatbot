...
with st.sidebar:
    ...
    if st.session_state.chat_history:
        ...
        for i, turn in enumerate(st.session_state.chat_history):
            preview = turn["user"][:35] + "..." if len(turn["user"]) > 35 else turn["user"]
            bot_preview = turn["bot"][:55] + "..." if len(turn["bot"]) > 55 else turn["bot"]

            with st.container(border=True):
                st.markdown(f"**Q: {preview}**")
                st.caption(f"A: {bot_preview}")
                if st.button("❌ Delete", key=f"del_{i}", use_container_width=True):
                    st.session_state.chat_history.pop(i)
                    msg_index = i * 2
                    if msg_index + 1 < len(st.session_state.messages):
                        st.session_state.messages.pop(msg_index + 1)
                        st.session_state.messages.pop(msg_index)
                    st.rerun()
    else:
